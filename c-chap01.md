...
Every C program must have a function named `main`, which serves as the entry point for the program.  Coming from Java, you might notice that main isn't `void` but instead should return an `int`.  By convention, `main` returns `0` if the program completed successfully. A non-zero return value indicates an unsuccessful completion or an error.

It is possible for `main` to take input arguments from the terminal and this is what the two arguments to main, `argc` and `argv` are used for.  More on these later.
...
In Java, it doesn't matter where the `main` method is placed in your source code.  This is because the compiler does some extra steps to make sure it understands the correct signature (what data type a function returns and what its argument data types are).  C doesn't do this.  Without getting too far into the weeds at this early stage, it's important to place your functions **above** your `main` function in your source code file.  In other words, you should declare and define your functions before you call them in another function (including `main`).
